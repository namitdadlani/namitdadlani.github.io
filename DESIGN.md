...
- **`/workshop`** — the odd one out, and the pattern to follow for anything similar going forward: it **links `../style.css`** and reuses the real nav, theme toggle, and footer markup verbatim from `index.html`, so it looks and behaves like a native part of the main site rather than a novelty. Only a small local `<style>` block is added per Workshop page, for the handful of bits genuinely specific to that page (e.g. the `.wk-card` grid on the index, or a demo's own controls/visualization styles). Individual experiments live at `/workshop/<demo-name>/`, link `../../style.css` and `../../favicon.svg`, and reuse the nav/footer markup with the corresponding `../../` and `../` relative paths. Each is plain vanilla JS/SVG/Canvas — no bundler, no backend, no API keys exposed in the browser — though a demo may load one CDN-hosted client-side library (e.g. Transformers.js) via a plain `<script>` tag if needed. Current demos ("Systems Lab"): `backoff` (retry/backoff timeline), `partitioning` (consistent-hashing ring) — each dramatizes a concept from the Experience section rather than shipping proprietary work code. (A third demo modeling the event-driven provisioning pipeline itself was built and then pulled — too close to the shape of actual AWS implementation details for comfort. Worth revisiting as an abstraction of a *different* project instead.)
